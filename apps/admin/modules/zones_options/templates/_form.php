...
<form action="<?php echo url_for('zones_options/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php endif; ?>
  <table>
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          &nbsp;<a href="<?php echo url_for('zones_options/index') ?>">Back to list</a>
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('Delete', 'zones_options/delete?id='.$form->getObject()->getId(), array('method' => 'delete', 'confirm' => 'Are you sure?')) ?>
          <?php endif; ?>
    ...
    <tbody>
      ...
      <tr>
        <th><?php echo $form['order_shipping_zones_options_list']->renderLabel() ?></th>
        <td>
          <?php echo $form['order_shipping_zones_options_list']->renderError() ?>
          <?php echo $form['order_shipping_zones_options_list'] ?>
        </td>
      </tr>
    </tbody>
  </table>
